...
let { time, execution } = start(
  [
    { file: "test", answer: 4512 },
    { file: "input", answer: 16716 },
  ],
  false
);
...
for (let i = 0; i < randomNumbers.length; i++) {
  ...
  let winningBoardIndex = boards.findIndex(x => x.isWinning());

  if (winningBoardIndex >= 0) {
    // console.log('Board ' + (winningBoardIndex + 1) + ' wins');
    winningScore = boards[winningBoardIndex].getScore() * value;
